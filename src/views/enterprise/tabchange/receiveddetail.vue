    ...
    <div class="container-wrap">
        ...
        <main class="post-info">
            ...
            <section class="post-desc" style="border: none">
                ...
                <div class="pd_post" v-for="item in applicationList" style="padding: 10px 26px">
                    <span>{{ item.post}} </span>
                    <span style="position: relative;top: -4px;padding: 0 5px">.</span>  
                    <span>{{item.provinceName }}{{item.cityName }}</span>
                    <span style="font-weight:700;float:right;color:#6eebdd"  v-show="item.expectedSalaryStart">{{ item.expectedSalaryStart + 'k~' + item.expectedSalaryEnd + 'k'}}</span>
                    <span style="font-weight:700;float:right;color:#6eebdd" v-show="!item.expectedSalaryStart">面议</span>
                </div>

                <div style="padding-left: 26px;" v-if='!applicationList.length'>
                    无
                </div>
        ...
        <footer class="post-detail-ft">
            <ul class="ft-ul">
                ...
                <li><x-button class='button' @click.native="delarchives">移出档案库</x-button></li>
            </ul>
        </footer>
        <div v-transfer-dom>
...
<script>
    ...
    import {myinfo,findOneCompany,addEmploy,delEmploy} from '@/api/company.js'
    ...
    import {formatDate} from '@/utils/date.js'
     ...
     export default {
        ...
        created(){
            ...
            if ( !this.$route.params.data.talentInfo) {
                return
            }
            let user = this.$route.params.data.talentInfo;
            
            let  resume = this.$route.params.data.talentInfo.resume
            ...
            // readSomOne(this.$route.query.id).then(res => {
                ...
                this.workingYear = resume.workingYear;
                this.userCity = user.city;
                this.age = resume.age;
                this.description = resume.description;
                this.us_name = resume.name;
                let sex = resume.sex;
                if(sex == 1){
                    this.us_sex = '男'
                }else if(sex == 2){
                ...
                this.birthday = resume.birthday;

                this.isOpen = resume.isOpen;
                if(this.isOpen == 0){
                    this.tel = "***********"
                }else{
                    this.tel =resume.tel
                };
                this.picUrl = resume.picUrl;
                this.licenseData = resume.licenseData;
                this.diplomasData = resume.diplomasData;
                this.applicationList = resume.applicationList;
                this.titleData = resume.titleData;

                // this.wantPost = recruiment.post;
                // this.wantPrivince= recruiment.provinceName;
                // this.wantCity = recruiment.cityName;
                // this.expectedSalaryStart = recruiment.salaryStart;
                // this.expectedSalaryEnd =  recruiment.salaryEnd;
                // this.jobCategoryName = recruiment.jobCategoryName;
                // this.mydescription = res.data.datas.description
              
                
                
                this.collectInfoId = this.$route.params.data.id;
                this.recruitmentId = this.$route.params.data.id;
            // })
            myinfo().then( res => {
